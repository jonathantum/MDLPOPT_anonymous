...
# ---------- imports ----------
from models.modules.tabular_encoder_MLP import MLPEncoder
from models.modules.regression_head import RegressionHead
from training.trainer import Trainer
from training.losses import masked_mse, weighted_masked_mae
...
#         return out
class TabularModel(nn.Module):
    def __init__(self, encoder, head, autoreg=False, teacher_forcing_ratio=0.5):
        """
        Args:
    ...
    def forward(self, X, X_mask=None, y=None, y_mask=None):
        ...
        B, T_in, F = X.shape
        device = X.device
        T_out = y.shape[1] if y is not None else self.head.T_out
        ...
        for t in range(T_out):
            ...
            # head takes last encoded step + previous prediction
...
def main():
    ...
    # load data
    X, X_mask, y, y_mask, ids = load_data("data/processed/dataset_2_or_less_missing/small_dataset_100")  # full_dataset small_dataset_10 small_dataset_50 small_dataset_100
    ...
    pipeline.train_loader = subsample_loader(pipeline.train_loader, 256)
    pipeline.val_loader    = subsample_loader(pipeline.val_loader, 32)
    pipeline.test_loader  = subsample_loader(pipeline.test_loader, 32)
    ...
    # ---------- trainer ----------
    trainer = Trainer(
        model=TabularModel(encoder, head, autoreg=True, teacher_forcing_ratio=0.9),
        optimizer=optimizer,
        loss_fn=weighted_masked_mae,
        device=device,
        cont_idx=cont_idx
    )
    ...
    # additional metrics
    y_true = np.concatenate([batch["y"] for batch in pipeline.test_loader], axis=0)
    ...
    # save predictions
    np.save("data/processed/test_predictions.npy", y_pred)
    print("Test predictions saved to data/processed/test_predictions.npy")
    ...
    for t in [2, 4, 6]:
        print(f"MAE >= {t}:", trainer.conditional_mae(y_true, y_pred, t))
    ...
    # metrics → flatten ONCE
    baseline_mae = mean_absolute_error(
        y_test.reshape(-1),
        y_baseline.reshape(-1)
    )

    baseline_rmse = np.sqrt(
        np.mean((y_test.reshape(-1) - y_baseline.reshape(-1)) ** 2)
    )
    ...
    print("Target percentiles:",
      np.percentile(y_true, [50, 80, 90, 95, 99]))
