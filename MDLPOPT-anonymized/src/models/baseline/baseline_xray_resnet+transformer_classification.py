...
# --- 2. Optimizer with Differential LR ---
    optimizer = torch.optim.AdamW([
        {"params": model.image_encoder.parameters(), "lr": 2e-5, "weight_decay": 1e-2},
    ...
    trainer = Trainer(
        ...
        device=device, task="wsi", wsi_threshold=wsi_threshold,
    ...
    labels, mask = trainer.derive_wsi_labels(y_train, torch.tensor(oai_pipeline.y_mask[oai_pipeline.train_idx]))
    counts = np.bincount(labels[mask].cpu().numpy().astype(int), minlength=3)
    weights = torch.tensor(len(labels[mask]) / (3.0 * counts), dtype=torch.float32).to(device)
    
    print(f"⚖️ Applying Class Weights: Improving={weights[0]:.2f}, Stable={weights[1]:.2f}, Worsening={weights[2]:.2f}")
    ...
    trainer = Trainer(
        model=model, 
        optimizer=optimizer, 
        loss_fn=None, # Trainer.step handles its own CE logic
        device=device, 
        task="wsi", 
    ...
    trainer.fit(oai_pipeline.train_loader, oai_pipeline.val_loader, epochs=100, patience=10, min_delta=0.005)
...
def main():
    ...
    loss, y_pred, y_probs, y_true = single_run_main(pipeline, device=device, wsi_threshold=1.0, input_years=3, target_years=1, label_aggregation="year_to_year")
    evaluate_wsi_classification(y_true, y_pred, y_probs, class_names=["Improving", "Stable", "Worsening"])
