...
from models.modules.mri_encoder import MRIEncoder # New Import

class MultimodalTemporalModel(nn.Module):
    def __init__(self, tab_encoder, mri_encoder, fusion_module, head):
        super().__init__()
    ...
    def forward(self, tabular, mri_dess=None, tabular_mask=None, mri_mask=None, **kwargs):
        B, T, F = tabular.shape
        
        # 1. Encode Tabular
        flat_tab = tabular.reshape(B * T, F)
        tab_feats = self.tab_encoder(flat_tab, mask=tabular_mask.reshape(B*T, F) if tabular_mask is not None else None)
        tab_feats = tab_feats.reshape(B, T, -1)
        
        # 2. Encode MRI (SAG_3D_DESS)
        if mri_dess is not None:
            # mri_dess shape: (B, T, D, H, W) or (B, T, 1, H, W)
            # Flatten B and T for the 2D/3D backbone
            _, _, D, H, W = mri_dess.shape
            flat_mri = mri_dess.reshape(B * T, D, H, W)
            mri_feats = self.mri_encoder(flat_mri) # (B*T, model_dim)
            mri_feats = mri_feats.reshape(B, T, -1)
            
            # 3. Simple Addition Fusion (or Concat + Linear)
            # We add them so the Transformer sees one "Year Token" containing both modalities
            combined_features = tab_feats + mri_feats
        else:
            combined_features = tab_feats

        # 4. Generate Transformer Mask (Ignore year if both modalities are missing)
        # Using tabular_mask as proxy for year existence
        transformer_mask = ~tabular_mask.any(dim=-1) if tabular_mask is not None else None

        # 5. Transformer Fusion & Head
        fused_vector = self.fusion_module(combined_features, transformer_mask)
...
def single_run_main(oai_pipeline, device="cuda", input_years=3, target_years=1):
    # Initialize Encoders
    tab_encoder = TabularYearEncoder(input_dim=67, model_dim=256)
    mri_encoder = MRIEncoder(model_dim=256) # Pretrained ResNet18
    
    fusion_layer = MidFusionTransformer(input_dim=256, num_layers=2)
    head = ClassificationHead(input_dim=256, n_classes=3)
    
    model = MultimodalTemporalModel(tab_encoder, mri_encoder, fusion_layer, head).to(device)
...
def main():
    ...
    X = np.load("data/processed/all_years/2_or_less_missing_target_visits/Womac_target_no_surgery/X.npy")
    X_mask = np.load("data/processed/all_years/2_or_less_missing_target_visits/Womac_target_no_surgery/X_mask.npy")
    y = np.load("data/processed/all_years/2_or_less_missing_target_visits/Womac_target_no_surgery/y.npy").squeeze(-1)
    y_mask = np.load("data/processed/all_years/2_or_less_missing_target_visits/Womac_target_no_surgery/y_mask.npy").squeeze(-1)
    ...
    ids = np.load("data/processed/all_years/2_or_less_missing_target_visits/Womac_target_no_surgery/ids.npy", allow_pickle=True)
mri_paths = np.load("mri_paths.npy", allow_pickle=True)
mri_mask = np.load("mri_mask.npy")

pipeline = OAIPipeline(X, X_mask, y, y_mask, xray_paths, xray_mask, ids, 
                        mri_paths=mri_paths, mri_mask=mri_mask)

# Modality must start with 'mri_' to match the logic in __getitem__
pipeline.run(modalities=["tabular", "mri_SAG_3D_DESS"], 
             batch_size=8, 
             xray_base_dir=xray_path,
             mri_base_dir=mri_path)
