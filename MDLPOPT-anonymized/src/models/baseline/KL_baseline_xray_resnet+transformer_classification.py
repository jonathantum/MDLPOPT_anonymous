...
# --- 2. Optimizer with Differential LR ---
    optimizer = torch.optim.AdamW([
        {"params": model.image_encoder.parameters(), "lr": 5e-5, "weight_decay": 1e-2},
    ...
    weights = torch.tensor([1.0, 15.0], dtype=torch.float32).to(device)
    print(f"⚖️ Binary Weights: Non-Prog={weights[0]:.2f}, Prog={weights[1]:.2f}")    #trainer.loss_fn = torch.nn.CrossEntropyLoss(weight=weights, label_smoothing=0.1, ignore_index=-100)
    ...
    trainer.fit(oai_pipeline.train_loader, oai_pipeline.val_loader, epochs=300, patience=10, min_delta=0.001)
...
def main():
    ...
    loss, y_pred, y_probs, y_true = single_run_main(pipeline, device=device, wsi_threshold=0.15, input_years=3, target_years=3, label_aggregation="max_delta")
